...
const SalesHistory = () => {
    ...
    const fetchSales = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`http://127.0.0.1:8000/sales?timeframe=${timeframe}`);
            setSales(res.data);
            setSelectedItems([]);
        } catch (err) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await axios.delete(`http://127.0.0.1:8000/sales/${itemToDelete.id}`);
    ...
    const confirmBulkDelete = async () => {
        ...
        try {
            await Promise.all(selectedItems.map(id => axios.delete(`http://127.0.0.1:8000/sales/${id}`)));
